{{-- components.tabs --}}

<permalink-tabs v-cloak inline-template>
    <div class="permalink-tabs">
        <ul class="nav nav-tabs pl-3" id="myTab" role="tablist">
            <li class="nav-item">
                <a class="nav-link active" id="meta-tab" data-toggle="tab" href="#meta" role="tab" aria-controls="meta" aria-selected="true">SEO</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" id="advanced-tab" data-toggle="tab" href="#advanced" role="tab" aria-controls="advanced" aria-selected="false">Robots</a>
            </li>
        </ul>
    </div>
</permalink-tabs>